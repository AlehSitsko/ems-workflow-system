...
def set_confirmation(api, call_id, status, note=None):
    body = {"confirmation_status": status}
    if note is not None:
        body["confirmation_note"] = note
    return api.patch(f"/api/calls/{call_id}/confirmation", json=body)


# ── Default state ───────────────────────────────────────────────────────────

def test_a_new_call_starts_as_not_called(client, roles, call):
    body = roles["dispatcher"].get(f"/api/calls/{call.id}").get_json()
...
def test_no_answer_is_distinct_from_not_called(client, roles, call):
    ...
    body = set_confirmation(roles["dispatcher"], call.id, "no_answer").get_json()
...
def test_resetting_to_not_called_clears_the_trail(client, roles, call):
    ...
    set_confirmation(roles["dispatcher"], call.id, "confirmed")
    body = set_confirmation(roles["dispatcher"], call.id, "not_called").get_json()
...
def test_an_alias_is_normalized(client, roles, call):
    body = set_confirmation(roles["dispatcher"], call.id, "voicemail").get_json()
    assert body["confirmation_status"] == "no_answer"


def test_an_unknown_status_is_rejected(client, roles, call):
    resp = set_confirmation(roles["dispatcher"], call.id, "maybe")
...
def test_an_overlong_note_is_rejected(client, roles, call):
    resp = set_confirmation(roles["dispatcher"], call.id, "confirmed", "x" * 1001)
    assert resp.status_code == 400
...
def test_declining_cancels_the_call_and_keeps_it_in_history(client, roles, call):
    body = set_confirmation(roles["dispatcher"], call.id, "declined",
...
def test_declining_without_a_note_still_records_a_reason(client, roles, call):
    ...
    body = set_confirmation(roles["dispatcher"], call.id, "declined").get_json()
    assert body["status"] == "cancelled"
    assert "declined" in body["cancel_reason"].lower()


def test_a_declined_call_leaves_the_open_board(client, roles, call):
    set_confirmation(roles["dispatcher"], call.id, "declined")

    board = roles["dispatcher"].get(f"/api/dispatch/board?date={TOMORROW}").get_json()
...
def test_declining_an_already_cancelled_call_is_allowed_and_idempotent(client, roles, call):
    set_confirmation(roles["dispatcher"], call.id, "declined", "First")
    body = set_confirmation(roles["dispatcher"], call.id, "declined", "Second").get_json()
...
def test_confirming_a_cancelled_call_is_refused(client, roles, call):
    ...
    roles["dispatcher"].patch(f"/api/calls/{call.id}/cancel", 
                 json={"cancel_reason": "Facility closed"})

    resp = set_confirmation(roles["dispatcher"], call.id, "confirmed")
...
def test_an_unknown_call_is_a_404(client, roles):
    assert set_confirmation(roles["dispatcher"], 999999, "confirmed").status_code == 404
...
def test_the_detail_endpoint_carries_a_minimized_patient_label(client, roles):
    ...
    body = roles["dispatcher"].get(f"/api/calls/{c.id}").get_json()
    assert body["patientLabel"] == "John D."


# ── The day's round ─────────────────────────────────────────────────────────

def round_for(api, day=TOMORROW):
    resp = api.get(f"/api/calls/confirmation-round?date={day}")
...
def test_the_round_lists_the_day_in_pickup_order(client, roles):
    ...
    times = [c["pickup_time"] for c in round_for(roles["dispatcher"])["calls"]]
...
def test_the_round_tallies_what_is_left(client, roles):
    ...
    summary = round_for(roles["dispatcher"])["summary"]
...
def test_cancelled_and_completed_trips_are_not_in_the_round(client, roles):
    ...
    assert round_for(roles["dispatcher"])["summary"]["total"] == 1


def test_declining_during_the_round_removes_it_from_the_round(client, roles, call):
    assert round_for(roles["dispatcher"])["summary"]["total"] == 1

    set_confirmation(roles["dispatcher"], call.id, "declined", "Not needed")
    assert round_for(roles["dispatcher"])["summary"]["total"] == 0


def test_the_round_validates_its_date(client, roles):
    assert roles["dispatcher"].get("/api/calls/confirmation-round?date=2099-02-30").status_code == 400
    assert roles["dispatcher"].get("/api/calls/confirmation-round").status_code == 400
