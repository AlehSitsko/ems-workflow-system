...
- [x] Refactor `DispatchBoardPage.jsx` into components/hooks — Phase 2d (complete, final phase)
  Priority: P1
  Area: frontend
  Done: Extracted the last and highest-risk slice — `UnitTable` (`frontend/src/components/dispatch/UnitTable.jsx`: every `onDragOver`/`onDragLeave`/`onDrop` drop-target handler, double-click status-advance, shift-severity styling, patient-queue sub-rows) and `UnitDetailPanel` (`frontend/src/components/dispatch/UnitDetailPanel.jsx`: renamed from the earlier aspirational `UnitDetailDrawer` since it's an inline panel, not an overlay — row-resize divider, status buttons, priority-queue banner/reset, assigned/completed call lists). The Unit Create/Edit `EntityDrawer` form, Make Night dialog, and all unit-CRUD/drag-and-drop handler *functions* stayed in the page — only the JSX invoking them moved. File went from 1,103 → 768 lines (2,439 → 768 total across all phases). Verified: `npm run lint`/`npm run build` clean (138 modules), `qa_test.py` 104/104 including its embedded 375-request load test, and an exhaustive manual browser pass — unit row select/deselect, double-click status advance, inline "→ Next Status" button, Edit Unit drawer, Make Night dialog, Out-of-Service toggle, drag-and-drop assignment (including the insufficient-crew warning modal and confirm), priority queue (Move Down, Reset to time order), Unassign, Mark Completed, and the bottom-panel resize divider — zero console errors throughout. This was the last remaining phase; the `DispatchBoardPage.jsx` refactor is now complete.
