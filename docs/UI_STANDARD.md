...
- **Hardcoded hex colors** still appear in several components (`CallForm.jsx`, `PatientOrderSection.jsx`, `PlannedUnitsList.jsx`, `CallDrawer.jsx`, `DocumentsTab.jsx`, `NotificationBell.jsx`, `PushNotificationBanner.jsx`, `BrowserNotificationSettings.jsx`) despite the design-token rule above. Needs a file-by-file review — some hits may be legitimate (SVG data URIs, one-off non-UI colors), not a blind find-and-replace. Operational *classification* colours are already tokenised (see above). The Dispatch Board's per-card status accents are an intentional self-contained pill palette, not this debt (see the Dispatch note above).
