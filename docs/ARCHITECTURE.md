...
- Performance indexes cover the hot query paths: `call` (trip_date, status, patient_id), `patient` (last_name, plus the encrypted-DOB **blind index** `dob_bidx` for exact DOB search/duplicate detection and the derived `dob_month_day` for the birthday calendar — `dob` itself is encrypted and deliberately **not** indexed in plaintext), `call_assignment` (unit_id, call_id, is_active), `daily_crew_unit` (shift_date), `user_notification` (user_id, is_read), `notification_event` (created_at), `time_entry` (employee_id), `audit_log` (entity_type, timestamp), `employee_document` (employee_id, expiry_date). The core perf indexes came from migration `a1b2c3d4e5f7`; the DOB blind/derived indexes from the field-encryption migration.
